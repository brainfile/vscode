import { BrainfileParser, type Column } from "@brainfile/core"
...
export class BrainfileCodeLensProvider implements vscode.CodeLensProvider {
	...
	public provideCodeLenses(
		...
		// Only process brainfile files (brainfile.md, .brainfile.md, .bb.md, brainfile.*.md)
		if (!this.isBrainfile(document.fileName)) {
			return []
		}
						...
						codeLenses.push(
							...
							new vscode.CodeLens(new vscode.Range(taskLocation, 0, taskLocation, 0), {
								...
								arguments: [
									...
									column.id,
									task.id,
									board.columns.map((c: Column) => ({ id: c.id, title: c.title })),
								],
	 ...
	 * - .brainfile.md
	 * - .bb.md
	 * - brainfile.*.md (e.g., brainfile.feature.md, brainfile.private.md)
	 */
	private isBrainfile(fileName: string): boolean {
