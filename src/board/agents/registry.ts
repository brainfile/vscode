...
import { AGENT_PROVIDERS, AgentProvider, getProvider } from "./providers";
 ...
 */
async function sendViaNativeChat(
 ...
 */
async function sendViaAddToContext(
  provider: AgentProvider,
  prompt: string
): Promise<SendResult> {
  ...
  if (commands.focusInput) {
    ...
    const pasteSuccess = await tryCommand(
 ...
 */
async function sendViaFocusPaste(
  provider: AgentProvider,
  prompt: string
): Promise<SendResult> {
  const { commands, focusDelay = 100 } = provider;
