 ...
 */
function isBrainfile(fileName: string): boolean {
...
const FIELD_DOCS: Record<string, { description: string; example?: string }> = {
	...
	schema: {
		...
		example: "schema: https://brainfile.md/v1/board.json",
	},
	agent: {
		description:
	...
	instructions: {
		description: "List of rules for AI agents to follow when modifying this brainfile.",
		example: `instructions:
  - Modify only the YAML frontmatter
  - Use task-[number] format for task IDs`,
	},
	rules: {
		description: "Behavioral rules organized by category: always, never, prefer, and context.",
	},
	always: {
  ...
  - id: 1
	...
	columns: {
		description: "List of kanban columns. Each column contains tasks and has an id, title, and order.",
		example: `columns:
  - id: todo
    title: To Do
    order: 1
	...
	statsConfig: {
		description: "Configuration for which columns appear in the board statistics display.",
		example: `statsConfig:
  columns:
    - todo
    - in-progress
	...
	// Column fields
	id: {
		description: "Unique identifier for columns and tasks. Use kebab-case (e.g., task-1, in-progress).",
		example: "id: task-1",
	},
	order: {
	...
	// Task fields
	description: {
		description: "Detailed description of the task. Supports markdown formatting.",
		example: `description: |
  ...
  - Support dark mode`,
	...
	template: {
		description: "Task template type. Built-in templates: feature, bug, chore, docs, refactor, test.",
...
export class BrainfileHoverProvider implements vscode.HoverProvider {
	...
	private isInFrontmatter(document: vscode.TextDocument, lineNumber: number): boolean {
		...
		return (
			frontmatterStart !== -1 && frontmatterEnd !== -1 && lineNumber > frontmatterStart && lineNumber < frontmatterEnd
		)
	}
