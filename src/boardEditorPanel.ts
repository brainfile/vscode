...
export class BoardEditorPanel {
  ...
  private async _handleDeleteRule(
    ruleId: string,
    ruleType: "always" | "never" | "prefer" | "context"
  ) {
    if (!this._boardFilePath) return;
  ...
  private async _handleFixIssues() {
    ...
    vscode.window.showInformationMessage(
      `Fixed ${result.issues.filter((i) => i.fixable).length} issue(s)!`
    );
    this._updateView();
  }
